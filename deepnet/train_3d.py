...
def eval(model,epoch):
    ...
    while i < count:
        ...
        action_size = len(sfpreds)
        tk = 2
        values, indices = torch.topk(sfpreds,k=tk,dim=0)
...
def train(episodes,log_every=100, model_every=1000):
    net = ThreeDeepNet().to(conf.DEVICE)
    net.train()
